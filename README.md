# Endgame

A static chess endgame practice room, published at https://junjiepeng.github.io/Chess-endgame-app/. Serve `dist/` using any local HTTP server. Chess rules use chess.js 1.4.0 and the automated opponent uses Stockfish.js 17.1 lite single-threaded in a Web Worker.

24 curated promotion, checkmate and defensive-draw exercises can be played as White or mirrored as Black. Features include English/Chinese, adjustable opponent skill (0–20), full-strength hints, optional evaluation and board aids, click or touch drag moves, arrows and marks, move review, undo, promotion choices, keyboard navigation, and move sounds. The library includes Lucena and Philidor, connected and protected passed pawns, bishop and knight endings, and rook/knight underpromotion. Search in either language, filter by theme, level or completion, or use Surprise me to prioritise unfinished positions.

The 24 exercises now contain 215 starting positions (191 newly added): nine per exercise, except eight for rook underpromotion. Each pool contains genuinely different, tablebase-verified positions: king distances, piece relationships and pawn advancement vary within the lesson's theme. Selection uses a shuffled deck per exercise, completes the deck before refilling, and avoids the last three positions across refills. The deck is saved on this device and included in backups. Board reflections/rotations provide additional orientations; they are not counted as distinct teaching positions. Hints follow the chosen position and orientation. Restart, Retry and changing colour retain the exact seed; selecting an exercise or Randomize draws another. Legacy saved games continue to use their original starting position.

Normal board clicks clear manual drawings outside touch Mark mode. Finishes show a green Success or red Unsuccessful result with the reason and a retry button. Conversion and defensive exercises can finish at a decisive material milestone after two full-strength engine checks, including an enemy rook won, a critical rook lost, or the last pawn removed from a balanced rook ending. Recaptures, promotion escapes and uncertain scores keep play going; dedicated mating-technique lessons still require mate. The confirmation is an engine estimate, not a tablebase verdict. Its position-bound evidence is saved and validated with the game, so results survive reloads without being re-awarded. Replies wait at least 950 ms after the player’s move is rendered, or 1200 ms when either side captures; engine thinking time counts toward this pause.

On phones, bottom navigation opens the endgame library, coaching panel, progress and settings without a long page scroll. Panels share the live controls and return directly to the board; the compact header, larger touch targets, safe-area spacing and 16px form fields support phone and installed-app use.

Settings, per-colour results (including successful defensive draws), daily completion streaks and the current game are saved locally. Your progress offers validated JSON backup export/import. Language and mute preferences are initially inherited from the opening trainer when available on the same origin; its progress and other settings are never changed. There is no cloud sync or account requirement.

Progress changes merge across open tabs using Web Locks where available. An idle tab preserves the latest saved game and settings. Damaged autosaved positions are discarded independently of valid results; manual backup imports remain strictly validated. Superseded queued engine analyses are skipped, and engine failures can be retried.

The installable app caches lessons, pieces and the complete engine for offline practice after the initial download. Font loading is optional and does not block practice. Third-party attributions and license links are in `dist/credits.html`.

## Publishing

The `Publish GitHub Pages` workflow deploys `dist/` on every push to `main`, and can also be run manually. Relative asset paths support GitHub Pages repository URLs and local preview.

**Bump the cache version in `dist/sw.js` whenever an app asset changes.** App assets are cached as a complete release; returning visitors see an update banner. Reloading from that banner first saves their game and activates the new worker. Cache cleanup is restricted to this app's `endgame-` prefix, so it does not affect the opening trainer.

The `.openai/hosting.json` file records the original, unpublished Sites registration; GitHub Pages is the active hosting destination.

## Lesson checks

All original and additional starting positions have been checked against the [Lichess tablebase](https://github.com/lichess-org/lila-tablebase). `tests/positions.json` stores the original positions. `tests/positions-{pawns,mates,specials}.json` stores the added FENs, tablebase outcomes, preserving moves and verification provenance. Position IDs must never be reassigned to different FENs, because saved games refer to them. Run `node --test tests/*.test.mjs` to check lesson integrity, mirrored positions, underpromotion, defensive draw scoring, backup compatibility, concurrent saves, long-game recovery, engine retries, app updates, shuffled decks, exact position restoration, pool diversity and both-colour legality of every orientation. These fixtures are authoring checks; practice still works completely offline.
