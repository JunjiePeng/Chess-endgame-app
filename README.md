...
Randomize changes the setup within the current exercise: pawn endings have two equivalent file reflections, and pawnless endings have eight board symmetries. Hints and saved games follow the selected setup; Restart repeats it. A normal board click clears manual drawings outside touch Mark mode. Finishes show a green Success or red Unsuccessful result with the reason and a retry button.
