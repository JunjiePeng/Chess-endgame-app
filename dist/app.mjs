...
let data=loadData(storage),prefs=data.preferences,t=translator(prefs.language);
...
let evalSequence=0,evalKey=null,evalValue=null,engineReady=false,offlineState='offlinePreparing',storageWarning=false,toastTimer;
...
function renderLibrary(){
 let group='';$('exercise-list').innerHTML=lessons.map(item=>{
  ...
  return heading+`<button class="exercise-option ${item.id===lesson.id?'active':''}" data-lesson="${item.id}" aria-pressed="${item.id===lesson.id}">${image(player+item.icon)}<span><strong>${lessonText(item,'title',prefs.language,player)}</strong><small>${done?t('lessonDone',{side:sideName(player)}):lessonText(item,'detail',prefs.language,player)}</small></span><span class="exercise-indicator" aria-hidden="true">${done?'✓':item.id===lesson.id?'›':'○'}</span></button>`;
 }).join('');
 $('solved-count').textContent=t('completed',{n:completedCount(player)});const days=streak(data.activity);$('streak-label').textContent=days?t('streak',{n:days}):'';
}
function localize(){
 ...
 $('exercise-category').textContent=text('group').toUpperCase()+` / 0${lessons.indexOf(lesson)+1}`;$('exercise-title').textContent=text('title');$('exercise-description').textContent=text('subtitle');$('difficulty').textContent=t(lesson.level==='Foundation'?'foundation':'intermediate');
 ...
 $('history-sides').textContent=t('white')+' / '+t('black');$('engine-note').textContent=t(engineReady?'engineNote':'engineRetry');$('offline-status').textContent=t(offlineState);
...
function render(){
 ...
 $('feedback').textContent=isReviewing()?t('reviewNotice'):feedback.key==='intro'?text('intro'):t(feedback.key,feedback.key==='blackPlayed'?{...feedback.args,goal:t(lesson.goal==='promotion'?'goalPromotion':'goalMate')}:feedback.args);
 ...
 $('next-btn').hidden=!finished||isReviewing();$('next-btn').textContent=t(completedCount(player)===6?'again':'next')+' →';
...
function checkOutcome(move){
 if(game.isCheckmate()){markFinished(game.turn()!==player?'winMate':'loseMate',game.turn()!==player);return true;}
 if(game.isDraw()){markFinished(game.isStalemate()?'stalemate':game.isInsufficientMaterial()?'materialDraw':game.isThreefoldRepetition()?'repetition':'fifty',false);return true;}
 if(lesson.goal==='promotion'&&move?.color===player&&move.promotion&&!game.moves({verbose:true}).some(m=>m.to===move.to&&m.captured)){markFinished('promoted',true);return true;}
 if(lesson.goal==='promotion'&&!game.board().flat().some(p=>p?.color===player&&p.type==='p')&&!game.board().flat().some(p=>p?.color===player&&['q','r'].includes(p.type))){markFinished('pawnGone',false);return true;}
 return false;
}
...
function renderProgress(){
 ...
 $('progress-summary').innerHTML=`<div><strong>${totalCompleted()} / 12</strong><span>${t('solved')}</span></div><div><strong>${wins}</strong><span>${t('wins')}</span></div><div><strong>${clean}</strong><span>${t('clean')}</span></div><p>${t('streak',{n:streak(data.activity)})}</p>`;
...
function readState(){return {lesson:lesson.id,player,fen:game.fen(),displayFen:viewGame().fen(),reviewPly,turn:game.turn(),thinking:busy,finished,success,moves:game.history(),legalMoves:game.moves(),preferences:{...prefs},completed:lessons.filter(l=>data.progress[`${l.id}:${player}`]?.wins).map(l=>l.id)};}
...
$('next-btn').onclick=()=>{const i=lessons.indexOf(lesson),next=lessons.slice(i+1).concat(lessons.slice(0,i+1)).find(l=>!data.progress[`${l.id}:${player}`]?.wins)||lessons[(i+1)%lessons.length];startLesson(next.id);};
...
$('confirm-import').onclick=()=>{if(!pendingImport)return;const snapshot=pendingImport.session;data=pendingImport;prefs=data.preferences;pendingImport=null;$('import-dialog').close();$('progress-dialog').close();restoreSession(snapshot);toast('importDone');};
...
const snapshot=data.session;restoreSession(snapshot);engine.init().then(()=>{engineReady=true;$('engine-note').textContent=t('engineNote');}).catch(()=>{$('engine-note').textContent=t('engineRetry');});
