...
export function wireBoardPointer({board,ghost,canMove,isMine,select,move,mark,onCancel,getRevision}){
 ...
 board.addEventListener('pointerup',e=>{if(!drag||e.pointerId!==drag.id)return;const d=drag;drag=null;cleanup();try{board.releasePointerCapture(e.pointerId);}catch{};suppressUntil=Date.now()+350;if(d.revision!==getRevision())return;const to=squareAt(e.clientX,e.clientY);if(d.right){if(to)mark(d.sq,to);return;}if(d.moved&&to&&to!==d.sq&&canMove())move(d.sq,to);});
 ...
 return {ignoreClick:e=>{const ignore=e.detail!==0&&Date.now()<suppressUntil;suppressUntil=0;return ignore;},cancel:()=>{drag=null;cleanup();}};
}
